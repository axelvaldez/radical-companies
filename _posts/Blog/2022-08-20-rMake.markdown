...
<h1>rMake</h1>
 <p>In the end, we opted to create a new app we call MAKE<sup id="fnref2"><a href="#fn2" rel="footnote">2</a></sup> and make it as an OSS platform that others can extend. There are several OSS editors that we can leverage for our UI. Also, there are many OSS components and libraries (<em>e.g.</em>, diff) that we can use in the making of rMake.</p>
 <p>For the backend, after trying to make our own, we are planning to use <a href="https://github.com/uprtcl">_Prtcl</a>, an OSS project, based on blockchain, that’s very much aligned with our goals.</p>
 <h2>No Trunk</h2>
